...
这比"哪种封面点击率高"更重要 —— 点击率高但语气不符，涨的是不会留下的粉，
而且读者点进来会有落差感。

所以选 `zine`（安静、克制、靠一个具体的东西承载情绪）还是
`plain` / `photo`（大字、直接、有冲击力），要看 `profile/voice.md`
里这个模式的实际语气。

⚠️ 档案里那一节标着 `无样本` 时，**不要猜她的语气来决定样式** ——
出两版给她选，问她哪一版更像自己的账号，然后把答案记回档案。
